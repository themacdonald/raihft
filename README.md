...
RAIHFT is an open-source toolkit that enables AI teams at recruiting companies to align and evaluate language and decision models using Reinforcement Learning from Human Feedback (RLHF). It provides a complete pipeline for collecting recruiter preferences (data labeling), training a reward model, fine-tuning models with RL, and analyzing model quality and fairness. By combining cutting-edge RLHF methods with bias detection and a user-centered interface, RAIHFT helps ensure recruitment AI systems behave as intended and ethically.
...
- **Data Labeling & Annotation**: Tools for recruiters to label candidate profiles or model responses (e.g. thumbs-up/down) to generate preference data for training.
...
- Run `python data_labeling_app.py` to start a web labeling interface for recruiters to compare AI outputs.
...
We welcome contributions! Please fork the repository, create a feature branch, and submit a pull request. Follow the style guidelines in `CONTRIBUTING.md`. Write unit tests for new functionality. Report issues via the GitHub Issue tracker. Engage with the community through discussions to help prioritize features.
